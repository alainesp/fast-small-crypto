...
extern "C" void sha1_block_plain_c(uint32_t state[5], const uint32_t W[16]) noexcept
{
	uint32_t E, A, C, D, B, W0, W1, W2, W3, W4, W5, W6, W7, W8, W9, W10, W11, W12, W13, W14, W15, t;

	// Load state
	E = state[4];	A = state[0];	C = state[2];	D = state[3];	B = state[1];
	...
	// Round 1
	E += rotl(A,  5);	E += (D ^ (B & (C ^ D)));	E += UINT32_C(0x5a827999);	E += W0;	B = rotl(B, 30);
	D += rotl(E,  5);	D += (C ^ (A & (B ^ C)));	D += UINT32_C(0x5a827999);	D += W1;	A = rotl(A, 30);
	C += rotl(D,  5);	C += (B ^ (E & (A ^ B)));	C += UINT32_C(0x5a827999);	C += W2;	E = rotl(E, 30);
	B += rotl(C,  5);	B += (A ^ (D & (E ^ A)));	B += UINT32_C(0x5a827999);	B += W3;	D = rotl(D, 30);
	A += rotl(B,  5);	A += (E ^ (C & (D ^ E)));	A += UINT32_C(0x5a827999);	A += W4;	C = rotl(C, 30);
	E += rotl(A,  5);	E += (D ^ (B & (C ^ D)));	E += UINT32_C(0x5a827999);	E += W5;	B = rotl(B, 30);
	D += rotl(E,  5);	D += (C ^ (A & (B ^ C)));	D += UINT32_C(0x5a827999);	D += W6;	A = rotl(A, 30);
	C += rotl(D,  5);	C += (B ^ (E & (A ^ B)));	C += UINT32_C(0x5a827999);	C += W7;	E = rotl(E, 30);
	B += rotl(C,  5);	B += (A ^ (D & (E ^ A)));	B += UINT32_C(0x5a827999);	B += W8;	D = rotl(D, 30);
	A += rotl(B,  5);	A += (E ^ (C & (D ^ E)));	A += UINT32_C(0x5a827999);	A += W9;	C = rotl(C, 30);
	E += rotl(A,  5);	E += (D ^ (B & (C ^ D)));	E += UINT32_C(0x5a827999);	E += W10;	B = rotl(B, 30);
	D += rotl(E,  5);	D += (C ^ (A & (B ^ C)));	D += UINT32_C(0x5a827999);	D += W11;	A = rotl(A, 30);
	C += rotl(D,  5);	C += (B ^ (E & (A ^ B)));	C += UINT32_C(0x5a827999);	C += W12;	E = rotl(E, 30);
	B += rotl(C,  5);	B += (A ^ (D & (E ^ A)));	B += UINT32_C(0x5a827999);	B += W13;	D = rotl(D, 30);
	A += rotl(B,  5);	A += (E ^ (C & (D ^ E)));	A += UINT32_C(0x5a827999);	A += W14;	C = rotl(C, 30);
	E += rotl(A,  5);	E += (D ^ (B & (C ^ D)));	E += UINT32_C(0x5a827999);	E += W15;	B = rotl(B, 30);
	t = (W2 ^ W13 ^ W8);	D += UINT32_C(0x5a827999);	W0 ^= t;	W0 = rotl(W0,  1);	D += rotl(E,  5);	D += (C ^ (A & (B ^ C)));	D += W0;	A = rotl(A, 30);
	t = (W3 ^ W14 ^ W9);	C += UINT32_C(0x5a827999);	W1 ^= t;	W1 = rotl(W1,  1);	C += rotl(D,  5);	C += (B ^ (E & (A ^ B)));	C += W1;	E = rotl(E, 30);
	t = (W4 ^ W15 ^ W10);	B += UINT32_C(0x5a827999);	W2 ^= t;	W2 = rotl(W2,  1);	B += rotl(C,  5);	B += (A ^ (D & (E ^ A)));	B += W2;	D = rotl(D, 30);
	t = (W5 ^ W0 ^ W11);	A += UINT32_C(0x5a827999);	W3 ^= t;	W3 = rotl(W3,  1);	A += rotl(B,  5);	A += (E ^ (C & (D ^ E)));	A += W3;	C = rotl(C, 30);

	// Round 2                                                
	t = (W6  ^ W1  ^ W12);	E += UINT32_C(0x6ed9eba1);	W4  ^= t;	W4  = rotl(W4 ,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W4;	B = rotl(B, 30);
	t = (W7  ^ W2  ^ W13);	D += UINT32_C(0x6ed9eba1);	W5  ^= t;	W5  = rotl(W5 ,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W5;	A = rotl(A, 30);
	t = (W8  ^ W3  ^ W14);	C += UINT32_C(0x6ed9eba1);	W6  ^= t;	W6  = rotl(W6 ,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W6;	E = rotl(E, 30);
	t = (W9  ^ W4  ^ W15);	B += UINT32_C(0x6ed9eba1);	W7  ^= t;	W7  = rotl(W7 ,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W7;	D = rotl(D, 30);
	t = (W10 ^ W5  ^ W0 );	A += UINT32_C(0x6ed9eba1);	W8  ^= t;	W8  = rotl(W8 ,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W8;	C = rotl(C, 30);
	t = (W11 ^ W6  ^ W1 );	E += UINT32_C(0x6ed9eba1);	W9  ^= t;	W9  = rotl(W9 ,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W9;	B = rotl(B, 30);
	t = (W12 ^ W7  ^ W2 );	D += UINT32_C(0x6ed9eba1);	W10 ^= t;	W10 = rotl(W10,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W10;	A = rotl(A, 30);
	t = (W13 ^ W8  ^ W3 );	C += UINT32_C(0x6ed9eba1);	W11 ^= t;	W11 = rotl(W11,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W11;	E = rotl(E, 30);
	t = (W14 ^ W9  ^ W4 );	B += UINT32_C(0x6ed9eba1);	W12 ^= t;	W12 = rotl(W12,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W12;	D = rotl(D, 30);
	t = (W15 ^ W10 ^ W5 );	A += UINT32_C(0x6ed9eba1);	W13 ^= t;	W13 = rotl(W13,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W13;	C = rotl(C, 30);
	t = (W0  ^ W11 ^ W6 );	E += UINT32_C(0x6ed9eba1);	W14 ^= t;	W14 = rotl(W14,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W14;	B = rotl(B, 30);
	t = (W1  ^ W12 ^ W7 );	D += UINT32_C(0x6ed9eba1);	W15 ^= t;	W15 = rotl(W15,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W15;	A = rotl(A, 30);
	t = (W2  ^ W13 ^ W8 );	C += UINT32_C(0x6ed9eba1);	W0  ^= t;	W0  = rotl(W0 ,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W0;	E = rotl(E, 30);
	t = (W3  ^ W14 ^ W9 );	B += UINT32_C(0x6ed9eba1);	W1  ^= t;	W1  = rotl(W1 ,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W1;	D = rotl(D, 30);
	t = (W4  ^ W15 ^ W10);	A += UINT32_C(0x6ed9eba1);	W2  ^= t;	W2  = rotl(W2 ,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W2;	C = rotl(C, 30);
	t = (W5  ^ W0  ^ W11);	E += UINT32_C(0x6ed9eba1);	W3  ^= t;	W3  = rotl(W3 ,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W3;	B = rotl(B, 30);
	t = (W6  ^ W1  ^ W12);	D += UINT32_C(0x6ed9eba1);	W4  ^= t;	W4  = rotl(W4 ,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W4;	A = rotl(A, 30);
	t = (W7  ^ W2  ^ W13);	C += UINT32_C(0x6ed9eba1);	W5  ^= t;	W5  = rotl(W5 ,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W5;	E = rotl(E, 30);
	t = (W8  ^ W3  ^ W14);	B += UINT32_C(0x6ed9eba1);	W6  ^= t;	W6  = rotl(W6 ,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W6;	D = rotl(D, 30);
	t = (W9  ^ W4  ^ W15);	A += UINT32_C(0x6ed9eba1);	W7  ^= t;	W7  = rotl(W7 ,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W7;	C = rotl(C, 30);

	// Round 3                                                
	t = (W10 ^ W5  ^ W0 );	E += UINT32_C(0x8f1bbcdc);	W8  ^= t;	W8  = rotl(W8 ,  1);	E += rotl(A,  5);	E += ((D & C) ^ (B & (D ^ C)));	E += W8;	B = rotl(B, 30);
	t = (W11 ^ W6  ^ W1 );	D += UINT32_C(0x8f1bbcdc);	W9  ^= t;	W9  = rotl(W9 ,  1);	D += rotl(E,  5);	D += ((C & B) ^ (A & (C ^ B)));	D += W9;	A = rotl(A, 30);
	t = (W12 ^ W7  ^ W2 );	C += UINT32_C(0x8f1bbcdc);	W10 ^= t;	W10 = rotl(W10,  1);	C += rotl(D,  5);	C += ((B & A) ^ (E & (B ^ A)));	C += W10;	E = rotl(E, 30);
	t = (W13 ^ W8  ^ W3 );	B += UINT32_C(0x8f1bbcdc);	W11 ^= t;	W11 = rotl(W11,  1);	B += rotl(C,  5);	B += ((A & E) ^ (D & (A ^ E)));	B += W11;	D = rotl(D, 30);
	t = (W14 ^ W9  ^ W4 );	A += UINT32_C(0x8f1bbcdc);	W12 ^= t;	W12 = rotl(W12,  1);	A += rotl(B,  5);	A += ((E & D) ^ (C & (E ^ D)));	A += W12;	C = rotl(C, 30);
	t = (W15 ^ W10 ^ W5 );	E += UINT32_C(0x8f1bbcdc);	W13 ^= t;	W13 = rotl(W13,  1);	E += rotl(A,  5);	E += ((D & C) ^ (B & (D ^ C)));	E += W13;	B = rotl(B, 30);
	t = (W0  ^ W11 ^ W6 );	D += UINT32_C(0x8f1bbcdc);	W14 ^= t;	W14 = rotl(W14,  1);	D += rotl(E,  5);	D += ((C & B) ^ (A & (C ^ B)));	D += W14;	A = rotl(A, 30);
	t = (W1  ^ W12 ^ W7 );	C += UINT32_C(0x8f1bbcdc);	W15 ^= t;	W15 = rotl(W15,  1);	C += rotl(D,  5);	C += ((B & A) ^ (E & (B ^ A)));	C += W15;	E = rotl(E, 30);
	t = (W2  ^ W13 ^ W8 );	B += UINT32_C(0x8f1bbcdc);	W0  ^= t;	W0  = rotl(W0 ,  1);	B += rotl(C,  5);	B += ((A & E) ^ (D & (A ^ E)));	B += W0;	D = rotl(D, 30);
	t = (W3  ^ W14 ^ W9 );	A += UINT32_C(0x8f1bbcdc);	W1  ^= t;	W1  = rotl(W1 ,  1);	A += rotl(B,  5);	A += ((E & D) ^ (C & (E ^ D)));	A += W1;	C = rotl(C, 30);
	t = (W4  ^ W15 ^ W10);	E += UINT32_C(0x8f1bbcdc);	W2  ^= t;	W2  = rotl(W2 ,  1);	E += rotl(A,  5);	E += ((D & C) ^ (B & (D ^ C)));	E += W2;	B = rotl(B, 30);
	t = (W5  ^ W0  ^ W11);	D += UINT32_C(0x8f1bbcdc);	W3  ^= t;	W3  = rotl(W3 ,  1);	D += rotl(E,  5);	D += ((C & B) ^ (A & (C ^ B)));	D += W3;	A = rotl(A, 30);
	t = (W6  ^ W1  ^ W12);	C += UINT32_C(0x8f1bbcdc);	W4  ^= t;	W4  = rotl(W4 ,  1);	C += rotl(D,  5);	C += ((B & A) ^ (E & (B ^ A)));	C += W4;	E = rotl(E, 30);
	t = (W7  ^ W2  ^ W13);	B += UINT32_C(0x8f1bbcdc);	W5  ^= t;	W5  = rotl(W5 ,  1);	B += rotl(C,  5);	B += ((A & E) ^ (D & (A ^ E)));	B += W5;	D = rotl(D, 30);
	t = (W8  ^ W3  ^ W14);	A += UINT32_C(0x8f1bbcdc);	W6  ^= t;	W6  = rotl(W6 ,  1);	A += rotl(B,  5);	A += ((E & D) ^ (C & (E ^ D)));	A += W6;	C = rotl(C, 30);
	t = (W9  ^ W4  ^ W15);	E += UINT32_C(0x8f1bbcdc);	W7  ^= t;	W7  = rotl(W7 ,  1);	E += rotl(A,  5);	E += ((D & C) ^ (B & (D ^ C)));	E += W7;	B = rotl(B, 30);
	t = (W10 ^ W5  ^ W0 );	D += UINT32_C(0x8f1bbcdc);	W8  ^= t;	W8  = rotl(W8 ,  1);	D += rotl(E,  5);	D += ((C & B) ^ (A & (C ^ B)));	D += W8;	A = rotl(A, 30);
	t = (W11 ^ W6  ^ W1 );	C += UINT32_C(0x8f1bbcdc);	W9  ^= t;	W9  = rotl(W9 ,  1);	C += rotl(D,  5);	C += ((B & A) ^ (E & (B ^ A)));	C += W9;	E = rotl(E, 30);
	t = (W12 ^ W7  ^ W2 );	B += UINT32_C(0x8f1bbcdc);	W10 ^= t;	W10 = rotl(W10,  1);	B += rotl(C,  5);	B += ((A & E) ^ (D & (A ^ E)));	B += W10;	D = rotl(D, 30);
	t = (W13 ^ W8  ^ W3 );	A += UINT32_C(0x8f1bbcdc);	W11 ^= t;	W11 = rotl(W11,  1);	A += rotl(B,  5);	A += ((E & D) ^ (C & (E ^ D)));	A += W11;	C = rotl(C, 30);

	// Round 4                                                
	t = (W14 ^ W9  ^ W4 );	E += UINT32_C(0xca62c1d6);	W12 ^= t;	W12 = rotl(W12,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W12;	B = rotl(B, 30);
	t = (W15 ^ W10 ^ W5 );	D += UINT32_C(0xca62c1d6);	W13 ^= t;	W13 = rotl(W13,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W13;	A = rotl(A, 30);
	t = (W0  ^ W11 ^ W6 );	C += UINT32_C(0xca62c1d6);	W14 ^= t;	W14 = rotl(W14,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W14;	E = rotl(E, 30);
	t = (W1  ^ W12 ^ W7 );	B += UINT32_C(0xca62c1d6);	W15 ^= t;	W15 = rotl(W15,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W15;	D = rotl(D, 30);
	t = (W2  ^ W13 ^ W8 );	A += UINT32_C(0xca62c1d6);	W0  ^= t;	W0  = rotl(W0 ,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W0;	C = rotl(C, 30);
	t = (W3  ^ W14 ^ W9 );	E += UINT32_C(0xca62c1d6);	W1  ^= t;	W1  = rotl(W1 ,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W1;	B = rotl(B, 30);
	t = (W4  ^ W15 ^ W10);	D += UINT32_C(0xca62c1d6);	W2  ^= t;	W2  = rotl(W2 ,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W2;	A = rotl(A, 30);
	t = (W5  ^ W0  ^ W11);	C += UINT32_C(0xca62c1d6);	W3  ^= t;	W3  = rotl(W3 ,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W3;	E = rotl(E, 30);
	t = (W6  ^ W1  ^ W12);	B += UINT32_C(0xca62c1d6);	W4  ^= t;	W4  = rotl(W4 ,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W4;	D = rotl(D, 30);
	t = (W7  ^ W2  ^ W13);	A += UINT32_C(0xca62c1d6);	W5  ^= t;	W5  = rotl(W5 ,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W5;	C = rotl(C, 30);
	t = (W8  ^ W3  ^ W14);	E += UINT32_C(0xca62c1d6);	W6  ^= t;	W6  = rotl(W6 ,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W6;	B = rotl(B, 30);
	t = (W9  ^ W4  ^ W15);	D += UINT32_C(0xca62c1d6);	W7  ^= t;	W7  = rotl(W7 ,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W7;	A = rotl(A, 30);
	t = (W10 ^ W5  ^ W0 );	C += UINT32_C(0xca62c1d6);	W8  ^= t;	W8  = rotl(W8 ,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W8;	E = rotl(E, 30);
	t = (W11 ^ W6  ^ W1 );	B += UINT32_C(0xca62c1d6);	W9  ^= t;	W9  = rotl(W9 ,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W9;	D = rotl(D, 30);
	t = (W12 ^ W7  ^ W2 );	A += UINT32_C(0xca62c1d6);	W10 ^= t;	W10 = rotl(W10,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W10;	C = rotl(C, 30);
	t = (W13 ^ W8  ^ W3 );	E += UINT32_C(0xca62c1d6);	W11 ^= t;	W11 = rotl(W11,  1);	E += rotl(A,  5);	E += (B ^ C ^ D);	E += W11;	B = rotl(B, 30);
	t = (W14 ^ W9  ^ W4 );	D += UINT32_C(0xca62c1d6);	W12 ^= t;	W12 = rotl(W12,  1);	D += rotl(E,  5);	D += (A ^ B ^ C);	D += W12;	A = rotl(A, 30);
	t = (W15 ^ W10 ^ W5 );	C += UINT32_C(0xca62c1d6);	W13 ^= t;	W13 = rotl(W13,  1);	C += rotl(D,  5);	C += (E ^ A ^ B);	C += W13;	E = rotl(E, 30);
	t = (W0  ^ W11 ^ W6 );	B += UINT32_C(0xca62c1d6);	W14 ^= t;	W14 = rotl(W14,  1);	B += rotl(C,  5);	B += (D ^ E ^ A);	B += W14;	D = rotl(D, 30);
	t = (W1  ^ W12 ^ W7 );	A += UINT32_C(0xca62c1d6);	W15 ^= t;	W15 = rotl(W15,  1);	A += rotl(B,  5);	A += (C ^ D ^ E);	A += W15;	C = rotl(C, 30);
...
/// <param name="state">The SHA1 state</param>
/// <param name="block">The message to compress</param>
extern "C" void sha1_block_sse2(simd::Vec128u32 state[5], const simd::Vec128u32 W[16]) noexcept
{
