...
extern "C" void md4_block_plain_c(uint32_t state[4], const uint32_t block[16]) noexcept
{
	...
	// Round 2
	for (int i = 0; i < 4; i++) {
		a += block[ 0];		t = d | c;		tt = c & d;		a += UINT32_C(0x5a827999);		t &= b;		t |= tt;		a += t;		a = rotl(a,  3);
		d += block[ 4];		t = c | b;		tt = b & c;		d += UINT32_C(0x5a827999);		t &= a;		t |= tt;		d += t;		d = rotl(d,  5);
		c += block[ 8];		t = b | a;		tt = a & b;		c += UINT32_C(0x5a827999);		t &= d;		t |= tt;		c += t;		c = rotl(c,  9);
		b += block[12];		t = a | d;		tt = d & a;		b += UINT32_C(0x5a827999);		t &= c;		t |= tt;		b += t;		b = rotl(b, 13);
		block +=  1;
	}
	block -=  4;

	// Round 3
	a += block[0];	t = d ^ c;	a += UINT32_C(0x6ed9eba1);	t ^= b;	a += t;		a = rotl(a,  3);
	d += block[8];	t = b ^ c;	d += UINT32_C(0x6ed9eba1);	t ^= a;	d += t;		d = rotl(d,  9);
	c += block[4];	t = a ^ b;	c += UINT32_C(0x6ed9eba1);	t ^= d;	c += t;		c = rotl(c, 11);
	b += block[12];	t = a ^ d;	b += UINT32_C(0x6ed9eba1);	t ^= c;	b += t;		b = rotl(b, 15);
	a += block[2];	t = d ^ c;	a += UINT32_C(0x6ed9eba1);	t ^= b;	a += t;		a = rotl(a,  3);
	d += block[10];	t = b ^ c;	d += UINT32_C(0x6ed9eba1);	t ^= a;	d += t;		d = rotl(d,  9);
	c += block[6];	t = a ^ b;	c += UINT32_C(0x6ed9eba1);	t ^= d;	c += t;		c = rotl(c, 11);
	b += block[14];	t = a ^ d;	b += UINT32_C(0x6ed9eba1);	t ^= c;	b += t;		b = rotl(b, 15);
	a += block[1];	t = d ^ c;	a += UINT32_C(0x6ed9eba1);	t ^= b;	a += t;		a = rotl(a,  3);
	d += block[9];	t = b ^ c;	d += UINT32_C(0x6ed9eba1);	t ^= a;	d += t;		d = rotl(d,  9);
	c += block[5];	t = a ^ b;	c += UINT32_C(0x6ed9eba1);	t ^= d;	c += t;		c = rotl(c, 11);
	b += block[13];	t = a ^ d;	b += UINT32_C(0x6ed9eba1);	t ^= c;	b += t;		b = rotl(b, 15);
	a += block[3];	t = d ^ c;	a += UINT32_C(0x6ed9eba1);	t ^= b;	a += t;		a = rotl(a,  3);
	d += block[11];	t = b ^ c;	d += UINT32_C(0x6ed9eba1);	t ^= a;	d += t;		d = rotl(d,  9);
	c += block[7];	t = a ^ b;	c += UINT32_C(0x6ed9eba1);	t ^= d;	c += t;		c = rotl(c, 11);
	b += block[15];	t = a ^ d;	b += UINT32_C(0x6ed9eba1);	t ^= c;	b += t;		b = rotl(b, 15);
