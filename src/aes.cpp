...
static const uint32_t round_constants[10] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };
...
#include <bit>
static inline uint32_t GET_UINT32_LE(const void* p) noexcept
{
    uint32_t r;
    memcpy(&r, p, sizeof(r));
...
static inline void PUT_UINT32_LE(void* p, uint32_t x) noexcept
{
    if constexpr (std::endian::native == std::endian::big)
        x = byteswap32(x);
...
#define AES256_ROUNDKEY_DWORDS ((240 + 16) / sizeof(uint32_t)) // 16 more bytes to simplify key generation

// This function produces 4*(Nr+1) round keys. The round keys are used in each round to decrypt the states. 
static void key_expansion_encryption(uint32_t* RK, const uint8_t* key, const unsigned num_rounds) noexcept
{
    ...
    case 14:
        ...
        for (int i = 0; i < 7; i++, RK += 8) {
            ...
            RK[10] = RK[2] ^ RK[ 9];
            RK[11] = RK[3] ^ RK[10];
            RK[12] = RK[4] ^ ((uint32_t)FSb[RK[11] & 0xff]) ^ ((uint32_t)FSb[(RK[11] >> 8) & 0xff] << 8) ^ ((uint32_t)FSb[(RK[11] >> 16) & 0xff] << 16) ^ ((uint32_t)FSb[RK[11] >> 24] << 24);
            RK[13] = RK[5] ^ RK[12];
            RK[14] = RK[6] ^ RK[13];
            RK[15] = RK[7] ^ RK[14];
        }
...
void AES_CTR_xcrypt(uint8_t* data, const size_t data_length, const uint8_t* key, const uint8_t key_length, const uint8_t iv[AES_BLOCKLEN]) noexcept
{
    ...
    for (i = 0, bi = AES_BLOCKLEN; i < data_length; ++i, ++bi)
    {
        if (bi == AES_BLOCKLEN) /* we need to regen xor compliment in buffer */
        {
            ...
            encrypt_block(buffer, round_key, num_rounds);
